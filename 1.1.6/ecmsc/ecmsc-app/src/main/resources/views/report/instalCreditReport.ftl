<#include "/layout.ftl">
	<@body>
			...
			<@panelBody>
				<@form id="queryForm" style="padding-top:10px;">
				   ...
				   <@field label="网点">
					...
					<@row>
					<@field label="起始日期">
						<@date id="instalCreditStartDate" value="${.now?date}" name="query.instalCreditStartDate"/>
					</@field>
					<@field label="截止日期">
						<@date id="instalCreditEndDate" value="${.now?date}" name="query.instalCreditEndDate"/>
					</@field>
				    </@row>
					<@toolbar align="right">
					<@button id="queryButton" name="查询" fa="search"/>
					<@button  name="导出" click="exportInstalCreditData"  fa="file-excel-o"/>
				    </@toolbar>
		 		</@form>
			 <@table id="dataTable" url="instalCreditReportController/list" form_id="queryForm" button_id="queryButton" condensed="true" before="checkkk()" load_auto="false">
		...
		<script type="text/javascript">
			function exportInstalCreditData(){
				var startDate = $("#instalCreditStartDate").val();
				var endDate = $("#instalCreditEndDate").val();
				...
				window.location.href="${base}/instalCreditReportController/exportInstalCreditData?startDate="+startDate+"&endDate="+endDate;
			}
			function checkkk(){
				var startDate = $("#instalCreditStartDate").val().split("-");
				var startTime = new Date(startDate[0],startDate[1],startDate[2]);
				var _startTime = startTime.getTime();
			//	alert(_startTime);
				
				var endDate = $("#instalCreditEndDate").val().split("-");
