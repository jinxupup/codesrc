<#include "/layout.ftl">
	<@body>
		<@panel head="标准大额分期放款成功明细报表">
  		...
  		<@field label="分期借款金额" >
  			<@input id="cashAmt" name="tmAppInstalLoan.cashAmt" value="${(tmAppInstalLoan.cashAmt)!}" valid={"regexp or pattern":"^((0|([1-9](\\d{1,14})?))(\\.\\d{1,2})?$)","regexp-message":"请正确填写借款金额"}/>
					...
					<@field label="查询起始日期">
						<@date id="successfulInstallmentStartDate" value="${.now?date}" name="query.successfulInstallmentStartDate"/>
					</@field>
					<@field label="查询截止日期">
						<@date id="successfulInstallmentEndDate" value="${.now?date}" name="query.successfulInstallmentEndDate"/>
					</@field>
				    </@row>
					<@toolbar align="right">
					<@button id="queryButton" name="查询" fa="search"/>
					<@button  name="导出" click="exportSuccessfulInstallmentData"  fa="file-excel-o"/>
				    </@toolbar>
		 		</@form>
			 <@table id="dataTable" url="successfulInstallmentReportController/list" form_id="queryForm" button_id="queryButton" page_size="10" condensed="true">
		...
		<script type="text/javascript">
			function exportSuccessfulInstallmentData(){
				var startDate = $("#successfulInstallmentStartDate").val();
				var endDate = $("#successfulInstallmentEndDate").val();
				if(!startDate || !endDate){
					alert("请起止日期再导出！");
					return;
				}
				window.location.href="${base}/successfulInstallmentReportController/exportSuccessfulInstallmentData?startDate="+startDate+"&endDate="+endDate;
			}
		</script>
	</@body>