<#include "/layout.ftl">
	...
	<@body>
		<@panel head="每日工作量汇总报表">
			<@panelBody>
				...
				<@row>
					...
					<@field label="员工">
						<@selectLink link_id="branch" url="instalWorkloadReportController/getUserByBranch?branch" keyfield="key" valuefield="value"
							showcode='true' nullable="false" name="query.user" />
					</@field>
				</@row>
				<@row>
					<@field label="查询起始日期">
						<@date id="instalWorkloadStartDate" value="${.now?date}" name="query.instalWorkloadStartDate"/>
					</@field>
					<@field label="查询截止日期">
						<@date id="instalWorkloadEndDate" value="${.now?date}" name="query.instalWorkloadEndDate"/>
					</@field>
				</@row>
					<@toolbar align="right">
					<@button id="queryButton" name="查询" fa="search"/>
					<@button  name="导出" click="exportInstalWorkloadData"  fa="file-excel-o"/>
				    </@toolbar>
		 		</@form>
			 <@table id="dataTable" url="instalWorkloadReportController/list" form_id="queryForm" button_id="queryButton" load_auto="false" condensed="true" before="checkkk()">
		...
		<script type="text/javascript">
			function exportInstalWorkloadData(){
				var startDate = $("#instalWorkloadStartDate").val();
				var endDate = $("#instalWorkloadEndDate").val();
				...
				window.location.href="${base}/instalWorkloadReportController/exportInstalWorkloadData?startDate="+startDate+"&endDate="+endDate;
			}
			function checkkk(){
				var startDate = $("#instalWorkloadStartDate").val().split("-");
				var startTime = new Date(startDate[0],startDate[1],startDate[2]);
				var _startTime = startTime.getTime();
			//	alert(_startTime);
				
				var endDate = $("#instalWorkloadEndDate").val().split("-");
