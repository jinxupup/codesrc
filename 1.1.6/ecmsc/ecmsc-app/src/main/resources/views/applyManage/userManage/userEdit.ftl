...
<@body>
	<@panel head="业务员信息">
		<@panelBody>
			...
			<@form id="form" action="manageRelation/editUser" before="beforeSubmit" success_url="manageRelation/userManagePage">
				...
				<@fieldset legend="状态信息">
					...
					<@row>
						<@field label="业务员">
							...
							options=ecms_('tableMap','getAllUser','') nullable="true" showfilter="true" single="true"  label_only="true"/>
						</@field>
						<@field label="业务员工号" hidden="true">
							<@input name="userNo"  value="${(user.userNo)!}" />
						</@field>
						<@field label="业务员姓名" hidden="true">
							<@input name="name"  value="${(user.name)!}" />
						</@field>
						<@field label="工作状态">
						<#--<@dictSelect dicttype="WORK_STATUS" name="condition" value="${(user.condition)!}"  valid={"stringlength":"true"}/>-->
							<@dictSelect dicttype="CURRENT_STATUS" name="condition" value="${(user.condition)!}" />
						</@field>
					</@row>
					<@row>
						<@field label="业务员类型">
