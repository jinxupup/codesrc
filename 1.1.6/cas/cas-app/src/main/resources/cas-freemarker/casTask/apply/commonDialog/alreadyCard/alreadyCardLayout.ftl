 			...
 			<#if alreadyCardsCardInfoDtos??>
	 			<#list alreadyCardsCardInfoDtos as alreadyCardsCardInfoDto>
	 				<tr>
	 					...
	 					<#--<td><@dictSelect options=cas_('tableMap','productForLowerLevel','${(alreadyCardsCardInfoDto.productCd)!}',
	 					'${(tmAppMain.appType)!}') value="${(alreadyCardsCardInfoDto.productCd)!}" label_only="true"/></td>-->
	 					<td><@select value="${(alreadyCardsCardInfoDto.productCd)!}" options=cas_('tableMap','productForStatus','A,B,C') label_only="true"/></td>
		...
		if(appNo == "" || appNo == null){
			...
			$.ajax({
				...
				success:function(res){
					if(res.s){
						window.open("${base}/cas_activiti/handleTask?appNo="+res.msg+"&detailFlag=Y");<#--跳转到申请详情页面-->
					}else{
						alert(res.msg);
					}
				}
