	...
	function ifSelectedCardChange(appType,num){
		...
		if(ifSelectedCard == 'Y'){
			...
			if(num != ''){
				...
				if($('#applyInputForm').data('bootstrapValidator').getOptions('attachCust['+num+'].cardNo','notEmpty') != null){
					$('#applyInputForm').data('bootstrapValidator').enableFieldValidators('attachCust['+num+'].cardNo',true,'notEmpty');
				}
			}else{
				$('#cardNo').bind('keyup',{'appType':appType},cardNoKeyup).bind('blur',{'appType':appType},cleanSelectCardNo);
				if(appType == 'A'){
					if($('#applyInputForm').data('bootstrapValidator').getOptions('primCust.cardNo','notEmpty') != null){
						$('#applyInputForm').data('bootstrapValidator').enableFieldValidators('primCust.cardNo',true,'notEmpty');
					}
				}
				if(appType == 'S'){
					if($('#applyInputForm').data('bootstrapValidator').getOptions('attachCust.cardNo','notEmpty') != null){
						$('#applyInputForm').data('bootstrapValidator').enableFieldValidators('attachCust.cardNo',true,'notEmpty');
					}
				}	
			}
		}else{
			...
			if(num != ''){
				...
				if(appType == 'A'){
					if($('#applyInputForm').data('bootstrapValidator').getOptions('primCust.cardNo','notEmpty') != null){
						$('#applyInputForm').data('bootstrapValidator').enableFieldValidators('primCust.cardNo',false,'notEmpty');
					}
				}
				if(appType == 'S'){
					if($('#applyInputForm').data('bootstrapValidator').getOptions('attachCust.cardNo','notEmpty') != null){
						$('#applyInputForm').data('bootstrapValidator').enableFieldValidators('attachCust.cardNo',false,'notEmpty');
					}
				}
			}
	...
	*/
	function getOrValidCardNo(productCd,cardNo,attachNo){
		...
		if(productCd != ''){
			$.ajax({
	     		type: 'POST',
	     		async:false,
				url: '${base}/cas_applyInput/getTmProduct',
		...
		if(cardNo != ''){
			...
			if(cardNo.length == ${(editCardNoLen)!9}){
				...
				if(validBit != 'false'){
					...
					if(num != ''){
						if($('#applyInputForm').data('bootstrapValidator').getOptions('attachCust['+num+'].cardNo') != null){
							$("#applyInputForm").data('bootstrapValidator').updateStatus('attachCust['+num+'].cardNo','NOT_VALIDATED');
						}
					}else{
						if(appType == 'A'){
							if($('#applyInputForm').data('bootstrapValidator').getOptions('primCust.cardNo') != null){
								$("#applyInputForm").data('bootstrapValidator').updateStatus('primCust.cardNo','NOT_VALIDATED');
							}
						}
						if(appType == 'S'){
							if($('#applyInputForm').data('bootstrapValidator').getOptions('attachCust.cardNo') != null){
								$("#applyInputForm").data('bootstrapValidator').updateStatus('attachCust.cardNo','NOT_VALIDATED');
							}
						}
					}
				}
	...
	function cleanSelectCardNo(event){
		...
		if(validBit == ''){
			$('#cardNo'+num).val('');
			if(num != ''){
				if($('#applyInputForm').data('bootstrapValidator').getOptions('attachCust['+idNum+'].cardNo') != null){
					$("#applyInputForm").data('bootstrapValidator').updateStatus('attachCust['+idNum+'].cardNo','NOT_VALIDATED');
				}
			}else{
				if(appType == 'S'){
					if($('#applyInputForm').data('bootstrapValidator').getOptions('attachCust.cardNo') != null){
	...
	function deleteSelectCardNo(appType,num){			
		...
		if(cardNo != ''){
			...
			$.ajax({
	     		type: 'POST',
				url: '${base}/cas_applyInput/cardNoDelete' ,
	...
	function idLastAllChange(appType,num){
		...
		if(idLastAll == 'Y'){
			if(appType == 'A'){
				if($('#applyInputForm').data('bootstrapValidator').getOptions('primCust.idLastDate','notEmpty') != null){
					$('#applyInputForm').data('bootstrapValidator').enableFieldValidators('primCust.idLastDate',false,'notEmpty');
				}
			}
			if(appType == 'S'){
				if(num != ''){
					if($('#applyInputForm').data('bootstrapValidator').getOptions('attachCust['+num+'].idLastDate','notEmpty') != null){
						$('#applyInputForm').data('bootstrapValidator').enableFieldValidators('attachCust['+num+'].idLastDate',false,'notEmpty');
					}
				}else{
					if($('#applyInputForm').data('bootstrapValidator').getOptions('attachCust.idLastDate','notEmpty') != null){
						$('#applyInputForm').data('bootstrapValidator').enableFieldValidators('attachCust.idLastDate',false,'notEmpty');
					}
				}
			}
			...
			if(appType == 'A'){
				if($('#applyInputForm').data('bootstrapValidator').getOptions('primCust.idLastDate','notEmpty') != null){
					$('#applyInputForm').data('bootstrapValidator').enableFieldValidators('primCust.idLastDate',true,'notEmpty').validateField('primCust.idLastDate');
				}
			}
			if(appType == 'S'){
				if(num != ''){
					if($('#applyInputForm').data('bootstrapValidator').getOptions('attachCust['+num+'].idLastDate','notEmpty') != null){
						$('#applyInputForm').data('bootstrapValidator').enableFieldValidators('attachCust['+num+'].idLastDate',true,'notEmpty').validateField('attachCust['+num+'].idLastDate');
					}
				}else{
					if($('#applyInputForm').data('bootstrapValidator').getOptions('attachCust.idLastDate','notEmpty') != null){
						$('#applyInputForm').data('bootstrapValidator').enableFieldValidators('attachCust.idLastDate',true,'notEmpty').validateField('attachCust.idLastDate');
					}
				}
			}
		}
	...
	*/
	function clickAjaxSelect(dicType,isCode,fieldType,that,parentId,defValue){
		...
		if(enableAjax=='true'){
			...
			$.ajax({
				type: 'POST',
				url : '${base}/cas_activiti/addrQuery',
	 ...
	 function selectAreaCode(){
		...
		if(idIssuerAddress.length>=4){
			$.ajax({
				...
				url: '${base}/cas_commonDialog/selectAreaByCode',
				success:function(res){
					if(res!=null){
						var result = res.obj;
