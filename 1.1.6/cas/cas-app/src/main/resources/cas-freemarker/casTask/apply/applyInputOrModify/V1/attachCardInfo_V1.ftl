<#if pageFieldMap['A1']??>
	<@fieldset legend="申请信息:">
		...
		<@row>
			<@field label="主卡卡产品代码">
				<@select id="productCd" readonly="true" name="tmAppMain.productCd" value="${(applyOperateDto.productCd)!}" options=cas_('tableMap','productForStatus','A') />
			</@field>
			<@field label="主卡卡面">
				<@select options=cas_("tableMap","productCardFace",'${(applyOperateDto.productCd)!}')
...
<#assign fieldsetMap = {'A2':'申请人基本信息','A5':'服务要求','A6':'银行专用栏'}>
