...
<#assign fieldsetMap = {'A2':'申请人基本信息','A3':'职业资料'}><#--,'A4':'联系人'-->
				...
				<#if fieldPageDto.componentType??>
					<#if fieldPageDto.componentType == "input">
						...
						<#if fieldPageDto.id == "ddBankAcctNo">
							...
							<div class="col-ar-8" style="padding-left:0px;">
								<@button id="checkCardBtn" name="验证" click="checkCard()"/>
							</div>
						<#else>
							<@input id="${(fieldPageDto.id)!}" readonly="${(fieldPageDto.ifReadonly)!}" name="${(fieldPageDto.name)!}" value="${(fieldPageDto.value)!}" />
						</#if>-->
...
<@row>
	<@field label="推广人注记" field_ar="36" label_ar="4" input_ar="32">
		<#if isEditMemo??>
			<@textarea name="query.memoInfo" rows="1" value="${(MEMOINPUT.memoInfo)!''}"/>
		<#else>
			<@textarea rows="1" value="${(MEMOINPUT.memoInfo)!''}" readonly="true"/>
		</#if>
	</@field>
</@row>
...
<script type="text/javascript">
	...
	$(function(){
		...
		if(idLastAll != '' && idLastAll == 'Y'){
			...
			$('#idLastDate').val('').attr('disabled','disabled');
		}
		...
		if(stmtMediaType != ''){
			if(stmtMediaType == 'P'){
				if($('#applyInputForm').data('bootstrapValidator').getOptions('primCust.email','notEmpty') != null){
					$('#applyInputForm').data('bootstrapValidator').enableFieldValidators('primCust.email', false,'notEmpty');
				}
			}
			if(stmtMediaType == 'E'){
	...
	<#--			$('#customerSpecialQuota').val(''); -->
		...
		$('#spreaderNo').blur(function(){
			...
			$.ajax({
	     		type: 'POST',
	     		async:false,
				url: '${base}/cas_applyInput/userIdValid' ,
	...
	var productCdChange = function(that){
				...
				if(attachNo != ''){
					...
					if(ifSelectedCard != '' && ifSelectedCard == 'Y'){
						...
						if(validBit != ''){
							...
							$('#cardNo'+i).val('');
							if($('#applyInputForm').data('bootstrapValidator').getOptions('attachCust['+i+'].cardNo') != null){
								$("#applyInputForm").data('bootstrapValidator').updateStatus('attachCust['+i+'].cardNo', 'NOT_VALIDATED');
							}
						}
					}
				}
		...
		if($('#isInstalment').length > 0){
			...
			if(isInstalment == "Y"){
				$.ajax({
		            url:"${base}/applyInput/getInstalParam",
		            type:"post",
		            dataType:"json",
	...
	function ddBankBranchChange(){
		...
		if(ddBankBranch != ''){
			$.ajax({
				url : "${base}/applyInput/getDdBankName",
				type : "post",
				dataType : "json",
	...
	var stmtMediaTypeChange = function(that){
		...
		if(stmtMediaType == 'P'){
			...
			if($('#applyInputForm').data('bootstrapValidator').getOptions('primCust.email','notEmpty') != null){
				$('#applyInputForm').data('bootstrapValidator').enableFieldValidators('primCust.email', true ,'notEmpty').validateField('primCust.email');
			}
	...
	function checkCard(){
		...
		if(ddBankAcctNo != ''){
			...
			$.ajax({
				...
				url: '${base}/cas_applyInput/checkDdBankAcct',
				success:function(res){
					alert(res.msg);
					if(!res.s){
						flag = false;
