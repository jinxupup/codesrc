...
<@panel>
        <@panelBody>
           ...
           <@table id="userList" url="cas_taskTransfer/getUserList?selectTask=${(selectTask)!}" single_select="true" form_id="queryForm" button_id="queryButton" pagination="false">
                <@th checkbox="true"/>
...
<script type="text/javascript">
		...
		$("#save").on("click",function(){
			...
			$.ajax({
				url:"${base}/cas_taskTransfer/judgeTransferOperaterUser",
				type:"post",
				...
				dataType:"json",
	 			success:function(ref){
	 				if(ref.s){
						 $.ajax({
			 				url:"${base}/cas_taskTransfer/transferTask?userNo="+ids.join(",") + "&selectTask=" + taskList,
		...
		var flashTransfer = function(){
			window.location.href="cas_taskTransfer/list";
