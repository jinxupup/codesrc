<#include "/layout.ftl"/>
<#include "../../casTask/apply/common/ajaxSelect.ftl"/>
<@body>
	...
	<#--共用js-->
	<#include "../../casTask/apply/common/pageCommonJs.ftl"/>
	<#--领卡方式的改变js-->
	<#include "../../casTask/apply/common/cardFetchJs.ftl"/>
		...
		<@panelBody>
			<@form id="applyInputForm">
				<@field hidden="true">
					...
					<@input name=" tmAppMain.accLmt" value="${(accLmt)!}"/>
  				</@field>
				<@tab style="height:340px;overflow-y:scroll;">
					<@tabNav>
			    	 	<#if appType??>
			    	 		<#if appType == "A">
			    		...
			    		<#if appType??>
			    	 		<#if appType == "A" || appType == "B" >
			        			<@tabPane id="pane1" active="true">
									<#include "../../casTask/apply/applyInputOrModify/V1/mainCardInfo_V1.ftl"/>
								</@tabPane>
								<@tabPane id="pane2" >
				     				<#include "../../casTask/apply/applyInputOrModify/V1/annexEviInfo_V1.ftl"/>
				        		</@tabPane>
				        		<@tabPane id="pane3" >
				     				<#include "../../casTask/apply/applyInputOrModify/V1/mainCardOtherInfo_V1.ftl"/>
				        		</@tabPane>
							<#elseif appType == "S">
								<@tabPane id="pane1" active="true">
									<#include "../../casTask/apply/applyInputOrModify/V1/attachCardInfo_V1.ftl"/>
								</@tabPane>
								<@tabPane id="pane3" >
				     				<#include "../../casTask/apply/applyInputOrModify/V1/attachCardOtherInfo_V1.ftl"/>
				        		</@tabPane>
			        		</#if>
			        	</#if>
			   		</@tabContent>
		 		</@tab>
				<#include "../../casTask/apply/common/applyCreditInfoBut_V1.ftl"/>
		 		<@fieldset legend="审批结果信息">
		        	<@row>
						...
						<@input id="accLmt" name="tmAppMain.accLmt" value="${(tmAppMain.accLmt)!}"
							readonly="${(
								applyOperateDto.rtfState=='K10' ||
								applyOperateDto.rtfState =='H21' ||
								applyOperateDto.rtfState =='Q05' ||
								applyOperateDto.rtfState =='L05'||
								applyOperateDto.rtfState =='N05'||
								applyOperateDto.rtfState =='H18')?
								string('false','true')}"/>
			        	</@field>
	...
	<@toolbar align="center" style="margin:0px; padding:0px; height:35px;width:98%;">
		<#if applyOperateDto.rtfState=='H18' || applyOperateDto.rtfState =='H21' || applyOperateDto.rtfState =='Q05' || applyOperateDto.rtfState =='L05'|| applyOperateDto.rtfState =='N05'>
			<@buttonAuth code="CAS_APPLY_ONLINEMAKECARD">
				<@ajaxButton confirm="确定实时建账制卡？" name="实时建账制卡" url="cas_onLineMakeCard/onLineMake?appNo=${(appNo)!}"/>
			</@buttonAuth>
		</#if>
		&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<script type="text/javascript">
		...
		var saveBtn = function(){
			$.ajax({
				...
				url: "${base}/cas_applyQuery/applyModifyInfoSave",
