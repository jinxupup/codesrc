...
@Controller
@RequestMapping("/cas_applyPreInput")
public class CasApplyPreInputController extends BaseController {
	 ...
	 * @return
	 */
	@RequestMapping("/applyPreInputpage")
	public String applyPreInputpage(Integer id){
		...
		return "apply/applyPreInput/applyPreInputPage.ftl";
	}
 ...
 */
	...
	public Json applyPreInputAdd(TmAppCustInfo cust){
					...
					if(CollectionUtils.sizeGtZero(list)){
						for (ApplyCardCustrInfoDto cardCustrInfoDto : list) {
							...
							if(StringUtils.isNotEmpty(bscSuppInd) && bscSuppInd.equals(AppConstant.bscSuppInd_B) 
								...
								List<TmProduct> tmProductList = productParamService.getTmProductByParam(tmProduct);
		...
		//是否实时发卡--由卡产品参数控制，业务人员不能控制；
//		if(tmAppMain.getProductCd()!=null){
//			TmProduct product = productService.getTmProductByCd(tmAppMain.getProductCd());
//			if(product!=null){
//				audit.setIsRealtimeIssuing(product.getIfRealtimeIssuing());
//			}else{
