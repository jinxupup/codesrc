...
public class CasDataController extends BaseController {
	...
	protected Query setBasicData(Query query,String taskState,String pageType,Logger logger) {
		...
		/*String a= getPara("isFreeTelCheck");*/
		...
		try {
			...
			//如果需要核准额度就去跟新TM_APP_MAIN表,以及修改信息历史
			if (appMainPage.getAccLmt()!=null){
				tmAppMainUpdate.setAccLmt(appMainPage.getAccLmt());
			}else if(tmAppMainUpdate.getAccLmt()==null){
				tmAppMainUpdate.setAccLmt(new BigDecimal(0));
			}
			/*if (appMainPage.getChkLmt()!=null){
				tmAppMainUpdate.setChkLmt(appMainPage.getChkLmt());
			}else {
				tmAppMainUpdate.setChkLmt(new BigDecimal(0));
			}*/
			...
			if (AppType.S.name().equals(appType)) {
				...
				if(tmAppContact1!=null&&(tmAppContact1.getContactName()!=null||
					...
					if(tmAppContactUpdate1 !=null) {
						modifiedFieldsMap = commonService.compareValue(tmAppContact1.convertToMap(), tmAppContactUpdate1.convertToMap(),
								appNo, "亲属" + AppConstant.TM_APP_CONTACT_NAME, taskState, fieldsMap.get(AppConstant.TM_APP_CONTACT));
						...
						modifiedFieldsMap = commonService.compareValue(tmAppContact1.convertToMap(),map,
								appNo, "其他"+AppConstant.TM_APP_CONTACT_NAME, taskState, fieldsMap.get(AppConstant.TM_APP_CONTACT));
		        ...
		        if(tmAppContact2!=null&&(tmAppContact2.getContactName()!=null||
		        	...
		        	if(tmAppContactUpdate2 != null){
						...
						tmAppContactUpdate2.setJpaVersion(tmAppContactUpdate1.getJpaVersion());
		        		tmAppContactUpdate2 = commonService.getModifiedClazz(tmAppContactUpdate2, modifiedFieldsMap);
		        	}
				...
				//TmAppAudit信息存储
				TmAppAudit tmAppAuditUpdate= applyInfoDto.getTmAppAudit();
				modifiedFieldsMap = commonService.compareValue(appAuditPage.convertToMap(),tmAppAuditUpdate.convertToMap(),
						appNo,AppConstant.TM_APP_AUDIT_NAME, taskState, fieldsMap.get(AppConstant.TM_APP_AUDIT));
		...
		if(isSubmit){
			...
			if(CollectionUtils.sizeGtZero(choiceCheckList)){
				for (int i = 0; i < choiceCheckList.size(); i++) {
					AllpyTelCheckRecordItem item = choiceCheckList.get(i);
					if(item==null){
						continue;
					}
