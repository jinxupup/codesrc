 ...
 */
@Component
public class ApplyInfoRiskToPreCheckUtils{
	 ...
	 * 重新风控
	 */
	public ApplyNodeCommonData reCellRiskPreExecute(String appNo) throws Exception {
		...
		dto.setAppNo(appNo);
		logger.info("重新调用风控系统for预审版本"+ AppConstant.BEGINING+ LogPrintUtils.printAppNoLog(appNo, start,null));
		dto = cellRiskPreExecute(start,dto,"重新调用客户经理推荐发卡版风控决策!");
		String remark = "";
		...
		logger.info("重新调用风控系统for预审版本"+AppConstant.END+LogPrintUtils.printAppNoEndLog(appNo, start,null));
	 ...
	 * @throws Exception
	 */
	public ApplyInfoDto cellRiskPreExecute(long start, ApplyInfoDto infoDto , String remark) throws Exception{
		...
		try {
			if (!StringUtils.equals(main.getAppType(), "S")) {
				...
				String riskPro = activitiUtils.getRiskProCd1ByProductAndAppSource(appNo, start,
						main.getProductCd(), main.getAppSource(), "RiskToPreCheck");
...
//								nextState = RtfState.H18;
			...
			nextState = RtfState.B16;//所有异常均跳入人工审批
