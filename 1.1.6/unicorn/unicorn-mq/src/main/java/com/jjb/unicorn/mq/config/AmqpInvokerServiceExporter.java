...
import com.jjb.unicorn.facility.util.RpcLogHandler;
import com.jjb.unicorn.facility.util.TradeIdWorker;
import com.jjb.unicorn.mq.exception.RPCShouldRequeueException;
...
public class AmqpInvokerServiceExporter extends RemoteInvocationBasedExporter implements MessageListener, InitializingBean {
	...
	public void onMessage(Message message) 
	{
		...
		if (content instanceof RemoteInvocation)
		{
		    ...
		    String LOC_SRC_TRADE_ID=AmqpContextHolder.getJydSrcTradeId();
		    
			if(forceMon){
				curr=TradeIdWorker.getInstance().getLocTraderId();
				
				// 打印请求报文的日志
				msg = String.format("TRADE_TYPE：[%s]，LOC_TRADE_ID：[%s]，LOC_SRC_TRADE_ID：[%s]", 
						serName+" "+request.getMethodName(), curr,LOC_SRC_TRADE_ID);
			...
			{
				AmqpContextHolder.setJydSrcTradeId(LOC_SRC_TRADE_ID);
				//发送同步调用的响应
