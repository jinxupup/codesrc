...
$(function(){
	$.fn.extend({
		unicornButtonDisable : function(disabled){
...
/**
 * unicorn 手动校验字段插件，例子 $("#formId").unicornValidField("name属性");
 * 参数 表单name 
 * @param $
 */
(function($){
	$.fn.unicornValidField = function(name){
		$(this).bootstrapValidator('updateStatus', name, 'NOT_VALIDATED').bootstrapValidator('validateField', name);
	};
})($);

/**
 * unicorn 手动校验表单插件,返回true（校验成功）或false（校验失败）
 * @param $
 */
(function($){
	$.fn.unicornValidForm = function(name){
...
/**
 * unicorn 元素滚动到底，父级不滚动 $("#domId").unicornScrollUnique();
 * @param $
 */
(function($){
	$.fn.unicornScrollUnique = function() {
...
(function($){
	...
	$.fn.unicornSelectBindHidden = function(isShow,options){
		...
		this.on('change',function(){
    		...
    		if(form.length>=1){
    			form.unicornValidField(willChangeName);
	...
	$(function () {
		/* unicorn-select-bind-hidden 开关 */
        $('[data-toggle="unicorn-select-bind-hidden"]').unicornSelectBindHidden();
        /* 注意：如果使用$('[name=xxx]').val(newVal)方式改了表单值，需要手动调用 onchange事件 $('[name=xxx]').trigger('change') */
    });
})($);

/**
 * unicorn 为checkbox添加隐藏字段插件
 * @param $
 */
(function($){
	...
	$.fn.unicornCheckboxBindHidden = function(isShow,options){
		var settings = $.extend({},DEFAULT, options);
	...
	$(function () {
		/* unicorn-checkbox-bind-hidden 开关 */
        $('[data-toggle="unicorn-checkbox-bind-hidden"]').unicornCheckboxBindHidden();
