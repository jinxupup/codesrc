...
public class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
    ...
    public ShardedJedisSentinelPool(String masters, String sentinels, String password) {
		this(masters, sentinels, new GenericObjectPoolConfig(),
...
//    }
    ...
    public ShardedJedisSentinelPool(String masters, String sentinels,
    	...
    	List<String> mlist_ = null;
    	if(StringUtils.isNotEmpty(masters)) {
    		mlist_ = Arrays.asList(masters.split(","));
    		
    	}
    	List<String> slist_ = null;
    	if(StringUtils.isNotEmpty(sentinels)) {
    		slist_ = Arrays.asList(sentinels.split(","));
    	}
		List<HostAndPort> masterList = initSentinels(slist_, mlist_);
		if(CollectionUtils.sizeGtZero(masterList)) {
			initPool(masterList);
		}
    }
	...
	private List<HostAndPort> initSentinels(List<String> sentinels, final List<String> masters) {
