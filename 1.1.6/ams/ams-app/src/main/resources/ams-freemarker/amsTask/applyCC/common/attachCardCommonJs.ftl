<script type="text/javascript">
    	...
    	if(idNo != ''){
    		...
    		if(idNo.length==15 || idNo.length==18){
    			if(idNoValid(idNo)){
					...
					if($('#applyInputForm').data('bootstrapValidator').getFieldElements('tmAppAttachApplierInfo['+num+'].gender') != null){
						$("#applyInputForm").data('bootstrapValidator').updateStatus('tmAppAttachApplierInfo['+num+'].gender','NOT_VALIDATED');
					}
					$('#gender'+num).attr({"readonly":"readonly","disabled":"disabled"});
					$('#birthday'+num).val(birthday);
					if($('#applyInputForm').data('bootstrapValidator').getFieldElements('tmAppAttachApplierInfo['+num+'].birthday') != null){
						$('#applyInputForm').data('bootstrapValidator').updateStatus('tmAppAttachApplierInfo['+num+'].birthday','NOT_VALIDATED');
					}
    			}else{
    				$('#idNo'+num).val('');
    				if($('#applyInputForm').data('bootstrapValidator').getFieldElements('tmAppAttachApplierInfo['+num+'].idNo') != null){
						$("#applyInputForm").data('bootstrapValidator').updateStatus('tmAppAttachApplierInfo['+num+'].idNo','NOT_VALIDATED');
					}
    			}
    		}
    	}
    ...
    function idTypeChange(num){
        ...
        if(idType == 'I'){
        	...
        	if($('#applyInputForm').data('bootstrapValidator').getFieldElements('tmAppAttachApplierInfo['+num+'].idNo','regexp') != null){
				$('#applyInputForm').data('bootstrapValidator').enableFieldValidators('tmAppAttachApplierInfo['+num+'].idNo',true,'regexp');
			}
        }else{
        	$('#idNo'+num).unbind('blur');
        	if($('#applyInputForm').data('bootstrapValidator').getFieldElements('tmAppAttachApplierInfo['+num+'].idNo','regexp') != null){
				$('#applyInputForm').data('bootstrapValidator').enableFieldValidators('tmAppAttachApplierInfo['+num+'].idNo',false,'regexp');
			}
        }
    }
	...
	function nameBlur(event){
		...
		if(name != ''){
			...
			if($('#applyInputForm').data('bootstrapValidator').getFieldElements('tmAppAttachApplierInfo['+num+'].embLogo') != null){
				$("#applyInputForm").data('bootstrapValidator').updateStatus('tmAppAttachApplierInfo['+num+'].embLogo','NOT_VALIDATED');
			}
		}	
	}
	<#--添加附卡初始化执行函数-->
	function initAttachCardInfo(num){
		...
		if(ifSelectedCard != '' && ifSelectedCard == 'Y'){
			...
			if($('#applyInputForm').data('bootstrapValidator').getFieldElements('tmAppAttachApplierInfo['+num+'].cardNo','notEmpty') != null){
		...
		if(idLastAll != '' && idLastAll == 'Y'){
			if($('#applyInputForm').data('bootstrapValidator').getFieldElements('tmAppAttachApplierInfo['+num+'].idLastDate','notEmpty') != null){
		...
		if(cardFetchMethod != ''){
			if(cardFetchMethod == 'B'){
				if($('#applyInputForm').data('bootstrapValidator').getFieldElements('tmAppAttachApplierInfo['+num+'].cardMailerInd','notEmpty') != null){
			...
			if(cardFetchMethod == 'A' || cardFetchMethod == 'E'){
				if($('#applyInputForm').data('bootstrapValidator').getFieldElements('tmAppAttachApplierInfo['+num+'].fetchBranch','notEmpty') != null){
