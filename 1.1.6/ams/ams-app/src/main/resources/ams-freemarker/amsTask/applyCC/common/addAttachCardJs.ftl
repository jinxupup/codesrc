<script type="text/javascript">
	...
	var addAttachCard = function(){
		...
		$('#card'+idNum).load("${base}/applyInput/addAttachCard #attach"+idNum,{'num':idNum,'productCd':$('#productCd').val()},function(){
			...
			<#--根据"自选卡号标志位"初始化"自选卡号"-->
			if($('#applyInputForm').data('bootstrapValidator').getOptions('tmAppAttachApplierInfo['+idNum+'].cardNo','notEmpty') != null){
				$('#applyInputForm').data('bootstrapValidator').enableFieldValidators('tmAppAttachApplierInfo['+idNum+'].cardNo', false,'notEmpty');
			}
