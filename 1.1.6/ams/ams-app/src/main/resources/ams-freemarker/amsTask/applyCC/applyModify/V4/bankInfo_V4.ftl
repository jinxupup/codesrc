<@fieldset legend="银行专用栏:">
	<@row>
		...
		<@field label="受理网点">
			...
			<@select id="owningBranch" name="tmAppMain.owningBranch" value="${(tmAppMain.owningBranch)!}"
        		options={}/>
		</@field>
	</@row>
	<@row>
	...
	<@row>
	 ...
	 <@row>
        <@field label="推广人一级网点名称" point_flag="${(otherInfoMap['spreaderBranchOne'])!}" point="*">			
			...
			<@select id="spreaderBranchOne" name="tmAppPrimCardInfo.spreaderBranchOne" value="${(tmAppPrimCardInfo.spreaderBranchOne)!}"
        		options={}/>			
		</@field>
...
<script type="text/javascript">
	$("#owningBranch").select2({
		 ...
		 language: "zh-CN",
		 ajax: { 
	        url: "${base}/ams_/getBranchesSelect",
	        dataType: 'json',
	        quietMillis: 250,
	...
	$.ajax({
	    type: 'GET',
	    url: '${base}/ams_/getBrancheByCode?branchCode=' + '${(tmAppMain.owningBranch)!}',
	    data:{},
	    dataType: 'json',
			success:function(res){
				if(res.s){
					var option = new Option(res.obj.key + "-" + res.obj.value, res.obj.key, true, true);
				    $("#owningBranch").append(option).trigger('change');
				    $("#owningBranch").trigger({
				        type: 'select2:select',
				        params: {
				            data: res
				        }
	...
	$.ajax({
	    type: 'GET',
	    url: '${base}/ams_/getBrancheByCode?branchCode=' + '${(tmAppPrimCardInfo.spreaderBranchOne)!}',
	    data:{},
	    dataType: 'json',
			success:function(res){
				if(res.s){
					var option = new Option(res.obj.key + "-" + res.obj.value, res.obj.key, true, true);
				    $("#spreaderBranchOne").append(option).trigger('change');
				    $("#spreaderBranchOne").trigger({
				        type: 'select2:select',
				        params: {
				            data: res
				        }
